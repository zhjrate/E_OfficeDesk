...
class QuotationBloc extends Bloc<QuotationEvents, QuotationStates> {
  ...
  @override
  Stream<QuotationStates> mapEventToState(
      QuotationEvents event) async* {
  ...
  Stream<QuotationStates> _mapQuotationListCallEventToState(
    ...
    try {
      ...
      await userRepository.getQuotationList(event.pageNo,event.quotationListApiRequest);
  ...
  Stream<QuotationStates> _mapQuotationNoToProductListCallEventToState(
    ...
    try {
      ...
      yield QuotationNoToProductListCallResponseState(event.StateCode,response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<QuotationStates> _mapInquiryProductSearchCallEventToState(
    ...
    try {
      ...
      yield InquiryProductSearchByStateCodeResponseState(event.ProductID,event.ProductName,event.Quantity,event.UnitRate,response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<QuotationStates> _mapQuotationHeaderSaveEventToState(
    ...
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      QuotationSaveHeaderResponse response = await userRepository.getQuotationHeaderSaveResponse(event.pkID,event.request);
      yield QuotationHeaderSaveResponseState(event.context,event.pkID,response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<QuotationStates> _mapQuotationProductSaveEventToState(
    ...
    try {
      ...
      QuotationProductSaveResponse respo = await userRepository
          .quotationProductSaveDetails(event.QT_No,event.quotationProductModel);
  ...
  Stream<QuotationStates> _mapDeleteQuotationCallEventToState(
    ...
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      QuotationDeleteResponse inquiryDeleteResponse = await userRepository.deleteQuotation(
          event.pkID, event.quotationDeleteRequest);
      yield QuotationDeleteCallResponseState(event.context,inquiryDeleteResponse);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<QuotationStates> _mapQuotationOtherChargeListEventToState(
    ...
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      QuotationOtherChargesListResponse quotationOtherChargesListResponse = await userRepository.getQuotationOtherChargeList(
          event.CompanyID, event.request);
      yield QuotationOtherChargeListResponseState(quotationOtherChargesListResponse);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
