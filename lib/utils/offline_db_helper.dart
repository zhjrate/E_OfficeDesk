...
class OfflineDbHelper {
  ...
  static const TABLE_PACKING_PRODUCT_ASSAMBLY_TABLE = "packing_product_assambly_table";
...
/*  static createInstance() async {
  ...
  static createInstance() async {
    ...
    database = await openDatabase(
      join(await getDatabasesPath(), 'soleoserp_database.db'),
      onCreate: (db, version) =>_createDb(db),version: 3
    );
  }

  static void _createDb(Database db) {

    db.execute('CREATE TABLE $TABLE_CONTACTS(id INTEGER PRIMARY KEY AUTOINCREMENT, pkId TEXT,CustomerID TEXT, ContactDesignationName TEXT, ContactDesigCode1 TEXT, CompanyId TEXT, ContactPerson1 TEXT, ContactNumber1 TEXT, ContactEmail1 TEXT, LoginUserID TEXT)',);
    db.execute('CREATE TABLE $TABLE_INQUIRY_PRODUCT(id INTEGER PRIMARY KEY AUTOINCREMENT, InquiryNo TEXT,LoginUserID TEXT, CompanyId TEXT, ProductName TEXT, ProductID TEXT, Quantity TEXT, UnitPrice TEXT,TotalAmount TEXT)',);
    db.execute('CREATE TABLE $TABLE_QUOTATION_PRODUCT(id INTEGER PRIMARY KEY AUTOINCREMENT, QuotationNo TEXT, ProductSpecification TEXT , ProductID INTEGER, ProductName TEXT, Unit TEXT, Quantity DOUBLE, UnitRate DOUBLE, DiscountPercent DOUBLE, DiscountAmt DOUBLE ,NetRate DOUBLE, Amount DOUBLE, TaxRate DOUBLE, TaxAmount DOUBLE, NetAmount DOUBLE, TaxType INTEGER, CGSTPer DOUBLE, SGSTPer DOUBLE, IGSTPer DOUBLE, CGSTAmt DOUBLE, SGSTAmt DOUBLE, IGSTAmt DOUBLE, StateCode INTEGER, pkID INTEGER, LoginUserID TEXT, CompanyId TEXT , BundleId INTEGER ,HeaderDiscAmt DOUBLE)',);
    db.execute('CREATE TABLE $TABLE_QUOTATION_OTHERCHARGE_TABLE(id INTEGER PRIMARY KEY AUTOINCREMENT,Headerdiscount DOUBLE,Tot_BasicAmt DOUBLE,OtherChargeWithTaxamt DOUBLE,Tot_GstAmt DOUBLE,OtherChargeExcludeTaxamt DOUBLE,Tot_NetAmount DOUBLE,ChargeID1 INTEGER,ChargeAmt1 DOUBLE,ChargeBasicAmt1 DOUBLE,ChargeGSTAmt1 DOUBLE,ChargeID2 INTEGER,ChargeAmt2 DOUBLE,ChargeBasicAmt2 DOUBLE,ChargeGSTAmt2 DOUBLE,ChargeID3 INTEGER,ChargeAmt3 DOUBLE,ChargeBasicAmt3 DOUBLE,ChargeGSTAmt3 DOUBLE,ChargeID4 INTEGER,ChargeAmt4 DOUBLE,ChargeBasicAmt4 DOUBLE,ChargeGSTAmt4 DOUBLE,ChargeID5 INTEGER,ChargeAmt5 DOUBLE,ChargeBasicAmt5 DOUBLE,ChargeGSTAmt5 DOUBLE)',);
    db.execute('CREATE TABLE $TABLE_PACKING_PRODUCT_ASSAMBLY_TABLE(id INTEGER PRIMARY KEY AUTOINCREMENT,PCNo TEXT,FinishProductID INTEGER,FinishProductName TEXT,ProductGroupID INTEGER,ProductGroupName TEXT,ProductID INTEGER,ProductName TEXT,Unit TEXT,Quantity DOUBLE,ProductSpecification TEXT,Remarks TEXT,LoginUserID TEXT,CompanyId TEXT)',);
    db.execute('CREATE TABLE $TABLE_FINAL_CHECKING_ITEM_TABLE(id INTEGER PRIMARY KEY AUTOINCREMENT,CheckingNo TEXT,CustomerID TEXT,Item TEXT,Checked TEXT,Remarks TEXT,SerialNo TEXT,SRno TEXT,LoginUserID TEXT,CompanyId TEXT)',);
  ...
  Future<List<InquiryProductModel>> getInquiryProduct() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(TABLE_INQUIRY_PRODUCT);
  ...
  Future<List<QuotationTable>> getQuotationProduct() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(TABLE_QUOTATION_PRODUCT);
...
*/
  ...
  Future<List<QT_OtherChargeTable>> getQuotationOtherCharge() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(TABLE_QUOTATION_OTHERCHARGE_TABLE);
  ...
  Future<int> insertPackingProductAssambly(PackingProductAssamblyTable model) async {
    final db = await database;
  ...
  Future<List<PackingProductAssamblyTable>> getPackingProductAssambly() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(TABLE_PACKING_PRODUCT_ASSAMBLY_TABLE);
  ...
  Future<void> updatePackingProductAssambly(PackingProductAssamblyTable model) async {
    final db = await database;
  ...
  Future<List<FinalCheckingItems>> getFinalCheckingItems() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(TABLE_FINAL_CHECKING_ITEM_TABLE);
