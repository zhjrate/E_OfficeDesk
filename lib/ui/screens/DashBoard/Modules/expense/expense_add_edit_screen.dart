                          ...
                          if(multiple_selectedImageFile.length!=0)
                            {
                              for(var i=0;i<multiple_selectedImageFile.length;i++)
                                {
                                  print("GetImageFileFromDevice : " +  "Path : " +multiple_selectedImageFile[i].path.toString() +"ImageName" +multiple_selectedImageFile[i].path.split('/').last);
  ...
  Widget showcustomdialogWithID1(String Category,
    ...
    return Container(
      child: Column(
        children: [
          InkWell(
            onTap: () => _expenseBloc..add(ExpenseTypeByNameCallEvent(
                ExpenseTypeAPIRequest(CompanyId: CompanyID.toString()))),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
      ...
      for(var i=0;i<multiple_selectedImageFile.length;i++)
        {
          _expenseBloc.add(ExpenseUploadImageNameCallEvent(
              ...
              ExpenseUploadImageAPIRequest(
                ...
                LoginUserId: LoginUserID,
                pkID: "0",
                ExpenseID: state.expsenseSaveResponse.details[0].column3.toString(),
...
*/
  ...
  void _onFetchImageListResponse(FetchImageListByExpensePKID_ResponseState state) async {
    arr_ImageList.clear();
    multiple_selectedImageFile.clear();
    if(state.fetchImageListByExpensePKID_Response.details.length!=0)
      {
        for(var i=0;i<state.fetchImageListByExpensePKID_Response.details.length;i++)
          {
            ALL_Name_ID all_name_id = ALL_Name_ID();
            all_name_id.Name = SiteURL+"otherimages/"+state.fetchImageListByExpensePKID_Response.details[i].name.toString();
            all_name_id.Name1 = state.fetchImageListByExpensePKID_Response.details[i].name.toString();
            arr_ImageList.add(all_name_id);
            File fetchimage = await _fileFromImageUrl(SiteURL+"otherimages/"+state.fetchImageListByExpensePKID_Response.details[i].name.toString(), state.fetchImageListByExpensePKID_Response.details[i].name.toString());
            /*NetworkToFileImage(
